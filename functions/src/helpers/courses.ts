...
export const getCourseDetail = (id: string): ICourseDetail => {
  const details: ICoursesDetail = {
    ...
    BlueBalboaParkGolfCourse: {
      ...
      hcp: [15, 11, 5, 1, 13, 3, 9, 17, 7, 4, 12, 6, 16, 8, 18, 2, 14, 10],
    ...
    nativeOaksBlack: {
      distance: [
        399, 531, 154, 411, 435, 377, 329, 175, 552, 384, 484, 375, 154, 435,
        350, 205, 410, 547,
      ],
      hcp: [9, 5, 17, 7, 1, 11, 13, 15, 3, 8, 4, 12, 18, 2, 16, 14, 10, 6],
      par: [4, 5, 3, 4, 4, 4, 4, 3, 5, 4, 5, 4, 3, 4, 4, 3, 4, 5],
      in: [3344, 36],
      out: [3363, 36],
      total: [6707, 72],
    },
    nativeOaksBlue: {
      distance: [
        380, 524, 129, 383, 419, 369, 315, 152, 528, 370, 465, 350, 140, 378,
        318, 181, 375, 539,
      ],
      hcp: [9, 5, 17, 7, 1, 11, 13, 15, 3, 8, 4, 12, 18, 2, 16, 14, 10, 6],
      par: [4, 5, 3, 4, 4, 4, 4, 3, 5, 4, 5, 4, 3, 4, 4, 3, 4, 5],
...
export const getFullCourses = () => {
  const courses = getCourses().courses as GolfCourse[];
  const fullCourses = courses.map((course) => {
    const newTeboxes = course.teeBoxes.map((teebox) => {
      const id = teebox.id;
      const teeboxDetails = getCourseDetail(id);
      const fullTeebox = { ...teebox, detail: teeboxDetails } as TeeBox;
      return fullTeebox;
    });
    const fullCourse = { ...course, teeBoxes: newTeboxes } as GolfCourse;
    return fullCourse;
  });
